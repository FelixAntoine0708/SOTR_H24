...
class field:
    def __init__(self):
        ...
        rospy.Subscriber("/robot_1/requete_shoot", std_msgs.msg.Bool, self.requestRedShoot)
        self.blueTeam = {
            "ballzone": False,
            "ball":0,
            "climbzone": False,
    ...
    def requestRedClimb(self, climbrobot_1):
        if climbrobot_1.data and self.redTeam['climbzone']:
                if self.redTeam['climbing'] and self.redTeam['stage'] < 3:
                    for x in tqdm(range(35), colour="RED", desc= f"Red Teams climb "):
                        s(0.1)
                    climb = randint(1, 100)
                    if climb <=60:
    ...
    def requestBlueShoot(self, shootrobot_0):
        if shootrobot_0.data:
                if self.blueTeam['ball'] >=1 and min(self.blueTeam['laser']) <=1.5:
                    self.blueTeam['ball'] -=1
                    for x in tqdm(range(20), colour="BLUE", desc= f"Blue Teams shoot"):
                        s(0.1)

                    climb = randint(1, 100)
                    if climb <= 80:
                        print("Success shoot Blue")
                        msg = 2
                        rate = rospy.Rate(10)
                        self.bluePoint.publish(msg)
                        msg = True
    ...
    def requestRedShoot(self, shootrobot_1):
        if shootrobot_1.data:
                if self.redTeam['ball'] >=1 and min(self.redTeam['laser']) <=1.5:
                    self.redTeam['ball'] -=1
                    for x in tqdm(range(10), colour="RED", desc= f"Red Teams shot"):
                        s(0.1)
                    climb = randint(1, 100)
                    if climb <=80:
                        print("Success shot Red")
                        msg = 2
                        rate = rospy.Rate(10)
                        self.redPoint.publish(msg)
                        msg = True
